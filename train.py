...
from lib.loss import ContrastiveLoss, Bilateral_ContrastiveLoss
...
def main():
    ...
    if opt.bilateral:
        criterion = Bilateral_ContrastiveLoss(opt=opt, margin=opt.margin, max_violation=opt.max_violation)
    else:
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        for i, (images, captions, lengths, ids) in enumerate(train_loader):
            ids = [element // 5 for element in ids]
